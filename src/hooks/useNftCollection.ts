...
const useNftCollection = (initialValue: NftCollection, setPutNftForSaleDialogVisible: Dispatch<SetStateAction<boolean>>) => {
  ...
  const nftCollectionContract = useMemo(() => (
    !!signer && !!provider ? new Contract(initialValue.nftContractAddr, NFTIMPLEMENT_ABI, signer) as NFT : null
  ), [signer, initialValue]);
  ...
  const mintNft = useCallback(async (name: string, description: string, image: Blob) => {
    if (!!nftCollectionContract) {
      ...
      try {
        ...
        const binaryObj = new TextEncoder().encode(JSON.stringify(metadataJson))
        const metadataUri = `data:application/json;base64,${encode(binaryObj)}`;
