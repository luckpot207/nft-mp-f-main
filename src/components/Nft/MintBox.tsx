import { background, Box, Button, Divider, Flex, Heading, Image, NumberDecrementStepper, NumberIncrementStepper, NumberInput, NumberInputField, NumberInputStepper, Skeleton, Text, useColorModeValue } from "@chakra-ui/react";
      ...
      <Flex as="section" padding={{ base: "4", md: "8" }} maxWidth="5xl" marginX="auto" flexDirection={{ base: "column", md: "row" }} flexWrap="nowrap">
        ...
        >
          <Image
            ...
            fallback={<Skeleton height="full" width="full" borderRadius={'10px'} />}
            border='20px solid'
            borderRadius={'15px'}
          />
        </Flex>
        ...
        <Box as="main" position="relative" minHeight={{ base: "auto", md: "full" }} width={{ base: "full", md: "50%" }} padding={{ base: "2", md: "4" }}>
          ...
          {/* Purchase */}
          <Heading fontSize={fs.lg} color={color.mainText} textAlign={{ base: "center", md: "start" }}>Mint your NativeNft</Heading>
          <Text marginTop="6" color={color.mainText}><b>NativeNft</b> is a wonderful token</Text>
          ...
          <Text marginTop="6" color={color.subText} fontStyle="italic">There is an upper limit to the total number of NativeNfts that can be minted, after which the only way to get more NativeNfts is to buy them from people who have put theirs for sale!</Text>
          <Text fontSize="2xl" color={color.button3} fontWeight={800} marginTop="6" textAlign={{ base: "center", md: "start" }}>1 NFT = 0.1 MATIC</Text>
          ...
          <Flex flexWrap="wrap" alignItems="center" gap={{ base: "4", md: "2" }} marginTop={{ base: "4", md: "2" }} width="fit-content" marginX={{ base: "auto", md: "unset" }} flexDirection={{ base: "column", md: "row" }}>
            ...
            <Button background={color.button2} color={color.mainText} leftIcon={<BuyIcon size="24" />} aria-label="Buy NativeNft for 0.1 matic" marginX={{ base: "auto", md: "unset" }} display="flex" onClick={() => mintNativeNft(BigNumber.from(mintNum))} isLoading={progressMintNativeNft} loadingText="Minting" disabled={isConnected && maxNativeNftsNum.eq(nativeNftMintedsNum)} width="fit-content">MINT</Button>

            {isConnected &&
              <Text color={color.subText} fontSize="md" textAlign={{ base: "center", md: "start" }}>{maxNativeNftsNum.sub(nativeNftMintedsNum).toString()} out of {maxNativeNftsNum.toString()} remaining!</Text>
            }
          </Flex>
        </Box>
      </Flex>
      ...
      {isConnected &&
        <>
          ...
          <Box as="section" padding={{ base: "4", md: "8" }} maxWidth="5xl" marginX="auto" marginTop="2">
            <Heading color={color.mainText} fontSize={fs.lg} textAlign="center" marginTop={{ base: "2", md: "unset" }}>Your NativeNfts</Heading>
