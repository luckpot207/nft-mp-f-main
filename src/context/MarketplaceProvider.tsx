...
import { NftContractCreatedEvent, AuctionContractCreatedEvent } from "../typechain-types/contracts/Marketplace";
import { MARKETPLACE_ABI, NFTIMPLEMENT_ABI, AUCTIONIMPLEMENT_ABI } from "../utils/abi";
...
interface MarketplaceContextProps {
  ...
  createNewAuction: (contract: string, tokenId: string, startingBid: string) => Promise<NFT | null | undefined>;
...
export const MarketplaceProvider = ({ children }: { children: ReactNode }) => {
  const { signer, provider, isConnected } = useEthConnection();
  ...
  const [progressCreateAuction, setProgressCreateAuction] = useState<boolean>(false);
  const toast = useToast();

  // Keep contracts updated
  useEffect(() => {
    if (!!provider) {
      const newMarketplaceContract: Marketplace = new Contract(MARKETPLACE_ADDR, MARKETPLACE_ABI, provider) as Marketplace;
      setMarketplaceContract(newMarketplaceContract);
    }
    if (!!signer) {
      const newMarketplaceContractConn: Marketplace = new Contract(MARKETPLACE_ADDR, MARKETPLACE_ABI, signer) as Marketplace;
  ...
  const createNewNftCollection = useCallback(async (name: string, symbol: string, description: string) => {
    if (!!marketplaceContractConn && signer) {
      try {
        ...
        const nftContractClone = new Contract(nftContractCloneAddr, NFTIMPLEMENT_ABI, signer) as NFT;
  ...
  // Function to create new NFT collection
  const createNewAuction = useCallback(async (contractAddress: string, tokenId: string, startingBid: string) => {
    if (!!marketplaceContractConn && signer) {
      try {
        setProgressCreateAuction(true);
        const txn = await marketplaceContractConn.createAuctionContract(contractAddress, tokenId, startingBid);
        const rcpt = await txn.wait();
        const event: AuctionContractCreatedEvent = rcpt.events?.find((event) => event.event === "AuctionContractCreated") as AuctionContractCreatedEvent;
        const auctionContractCloneAddr = event.args.contractAddr;
        const auctionContractClone = new Contract(auctionContractCloneAddr, AUCTIONIMPLEMENT_ABI, signer) as NFT;
        await refreshLoadedData();
